...
extension EWAddressPickView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.label.text = dataArray[indexPath.row - 1]
        return cell
    }
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .provinces:
            ...
            selectedProvince = (locationModel?.provincesArray[indexPath.row - 1]) ?? ""
            self.provincesModel = locationModel?.countryDictionary[selectedProvince]
            self.tableViewType = .city
        case .city:
            /// 当前为选择城市状态时,保存选中城市,刷新状态为选择地区
            selectedCity = (provincesModel?.cityArray[indexPath.row - 1]) ?? ""
            self.cityModel = provincesModel?.provincesDictionary[selectedCity]
            self.tableViewType = .area
        case .area:
            /// 当前为选择地区状态时,保存选中地区,执行回调block.将选中数据回调
            selectedArea = dataArray[indexPath.row - 1]
